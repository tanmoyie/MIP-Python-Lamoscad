...
# %% multiscenario
def analyze_multiscene(model, Stations, OilSpills, ResourcesD, Demand, deploy, select):
    ...
    model.ScenNName = 'Increased spill quantity'
    for o in OilSpills:
        for r in ResourcesD:
            C_deploy_demand[o, r].ScenNRhs = Demand[o, r] * 1.2
    """
    """
    C_deploy_demand = model.addConstrs((gp.quicksum(deploy[s, o, r] for s in Stations)
    ...
    model.ScenNName = 'Double spill/demands'
    for o in OilSpills:
        for r in ResourcesD:
            C_deploy_demand[o, r].ScenNRhs = Demand[o, r] * 2.0


    # %%
    # Print solution for each scenario
    for scene in range(model.NumScenarios):
        # Set the scenario number to query the information for this scenario
        model.Params.ScenarioNumber = scene

        print('\n\n------ Scenario %d (%s)' % (scene, model.ScenNName))

        # Check if we found a feasible solution for this scenario
        if model.ModelSense * model.ScenNObjVal >= GRB.INFINITY:
            if model.ModelSense * model.ScenNObjBound >= GRB.INFINITY:
            ...
            print('\nObjective values considering RE: %g' % model.ScenNObjVal)
            print('SOLUTION:')
            for s in Stations:
                for r in ResourcesD:
                    if select[s, r].ScenNX > 0.5:
                        print('Station %s open' % s)
                        for o in OilSpills:
                            # for r in ResourcesD:
                            if deploy[s, o, r].ScenNX > 0:
                                # print('  Deploy {deploy[s, o, r].ScenNX:g} units to oil spill zone {o}')
                                print('  Deploy %g units to oil spill zone %s' %
                                      (deploy[s, o, r].ScenNX, o))
                            else:
                                print('Station %s closed!' % s)

    # Print a summary table: for each scenario we add a single summary line
    print('\n\nSummary: Closed station depending on scenario\n')
    tableStr = '%8s | %17s %13s' % ('', 'Station', '|')
    print(tableStr)

    tableStr = '%8s |' % 'Scenario'
    for s in Stations:
        tableStr = tableStr + ' %s' % s
...
m.ScenNName = 'Increased warehouse demands by 10%'
for w in warehouses:
...
for s in range(m.NumScenarios):
    ...
    if m.ModelSense * m.ScenNObjVal >= GRB.INFINITY:
        ...
        # motivated from https://www.gurobi.com/documentation/current/examples/multiscenario_py.html
