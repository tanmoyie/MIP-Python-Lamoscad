...
def solve_primal_subproblem(OilSpills, Stations, Resources, Vehicles,
    ...
    b_os = model.addVars(OilSpills, Stations, vtype=GRB.CONTINUOUS, name="b_os")  # BINARY
    ...
    con_alpha = model.addConstrs(quicksum(z[s, o, r] for o in OilSpills) <= A_sr[s, r] * x_bar[s] for s in Stations for r in Resources)
    con_beta = model.addConstrs(quicksum(z[s, o, r] for s in Stations) <= demand_or[o, r] * sum(y_bar[o, s] for s in Stations) for o in OilSpills for r in Resources)
    con_gamma = model.addConstrs(quicksum(z[s, o, r] for o in OilSpills for r in Resources) >= nQ for s in Stations)
