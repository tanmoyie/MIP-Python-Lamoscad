...
class ModelW:
    def solve(Stations, OilSpills, ResourcesD, coordinates_st, coordinates_spill, SizeSpill, SizeSpill_n,
        ...
        C_resource_capacity = model.addConstrs((deploy.sum('*', s, r) <= BigM * Availability[s, r] * select[s]  #
                                                for s, r in sr_pair),
                                               name='C_open_facility')  # need improvement++ infeasibility
        ...
        objective_1 = gp.quicksum((w1 * SizeSpill_n[o] + w2 * Sensitivity_n[o] - w3 * Distance_n[o, s]) * cover[o, s]
                                  for o, s in os_pair)

        objective_2 = gp.quicksum(w4 * select[s] * Cf_s[s] for s in st_o) \
                      + gp.quicksum(
            (w5 * CostU[s, r] - w6 * Effectiveness_n[s, r]) * deploy[o, s, r] for o, s, r in osr_pair)
        ...
        values = model.getAttr('X',
                               mvars)  # X Xn https://www.gurobi.com/documentation/9.5/refman/working_with_multiple_obje.html
