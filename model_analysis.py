...
# %% Network Diagram
def draw_network_diagram(DistanceMax, NumberStMax, Sensitivity_R, spill_df, station_df, ResponseTimeT, coverage_percentage,
    ...
    for ust in range(len(unique_stations)):
        ...
        lc = mc.LineCollection(new_list, colors=f'C{ust + 1}',
                               alpha=.3, linewidths=3)  # 'Resource Type' alpha = (ust/len(unique_stations)), colors=ust,
    ...
    st = plt.scatter(data=station_df[station_df['Station no.'].isin(selected_supply_stations)],
                     ...
                     marker=shape, alpha=alpha_val, s=250,
                     edgecolors='black',
    ...
    for i in range(len(selected_supply_stations)):
        plt.text(x=data_st_selected.St_Longitude[i] + 2.5, y=data_st_selected.St_Latitude[i] - .25,
